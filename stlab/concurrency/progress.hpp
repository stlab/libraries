...
namespace stlab
{
    namespace detail 
    {
        ...
        public:
            ...
            tracker_server(std::function<void(size_t, size_t)> f)
                : _signal(std::move(f)) 
            {}
        ...
        struct tracker_client
        {
            ...
            tracker_client(const tracker_client& x) 
                : _f(x._f)
                , _p(x._p) {
                auto p = _p.lock();
                if (p) p->add_task();
            ...
            tracker_client(std::shared_ptr<tracker_server>& p, F&& f)
                : _f(std::move(f))
                , _p(p) {
                p->add_task();
            }
    ...
    public:
        progress_tracker() :
            _tracker(std::make_shared<detail::tracker_server>())
        {}
