...
BOOST_AUTO_TEST_CASE(future_get_wait_copyable_value) {
    BOOST_TEST_MESSAGE("future get_wait with copyable value");
    auto answer = [] { return 42; };

    stlab::future<int> f = stlab::async(stlab::default_executor, answer);

    BOOST_REQUIRE_EQUAL(42, stlab::get_wait(f));
}

BOOST_AUTO_TEST_CASE(future_get_wait_moveonly_value) {
    BOOST_TEST_MESSAGE("future get_wait with moveonly value");
    auto answer = [] { return stlab::move_only(42); };

    stlab::future<stlab::move_only> f = stlab::async(stlab::default_executor, answer);

    BOOST_REQUIRE_EQUAL(42, stlab::get_wait(std::move(f)).member());
}

BOOST_AUTO_TEST_CASE(future_get_wait_void) {
    BOOST_TEST_MESSAGE("future get_wait with void");
    int v = 0;
    auto answer = [&] { v = 42; };
...
BOOST_AUTO_TEST_CASE(future_get_wait_void_with_timeout) {
    BOOST_TEST_MESSAGE("future get_wait with void");
    int v = 0;
    auto answer = [&] { v = 42; };
    ...
    auto r = stlab::get_wait_for(f, std::chrono::seconds(2));
    BOOST_REQUIRE_EQUAL(42, v);
    BOOST_REQUIRE_EQUAL(true, r.is_ready());
}

BOOST_AUTO_TEST_CASE(future_get_wait_void_with_timeout_reached) {
    BOOST_TEST_MESSAGE("future get_wait with void with a timeout");
    auto answer = [&] {
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
    };

    stlab::future<void> f = stlab::async(stlab::default_executor, answer);
    f = stlab::get_wait_for(std::move(f), std::chrono::milliseconds(100));
    BOOST_REQUIRE(!f.is_ready());
    stlab::get_wait(f);
}

BOOST_AUTO_TEST_CASE(future_get_wait_int_with_timeout_reached) {
    BOOST_TEST_MESSAGE("future get_wait with int with a timeout");
    auto answer = [&] {
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
        return 42;
    };
    ...
    f = stlab::get_wait_for(std::move(f), std::chrono::milliseconds(100));
    BOOST_REQUIRE(!f.is_ready());
    BOOST_REQUIRE(stlab::get_wait(f) == 42);
}

BOOST_AUTO_TEST_CASE(future_get_wait_move_only_with_timeout_reached) {
    BOOST_TEST_MESSAGE("future get_wait with move_only with a timeout");
    auto answer = [&] {
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
        return move_only(42);
    };
    ...
    f = stlab::get_wait_for(std::move(f), std::chrono::milliseconds(100));
    BOOST_REQUIRE(!f.is_ready());
    BOOST_REQUIRE(stlab::get_wait(std::move(f)).member() == 42);
}

BOOST_AUTO_TEST_CASE(future_get_wait_copyable_value_error_case) {
    BOOST_TEST_MESSAGE("future get_wait with copyable value error case");
    auto answer = []() -> int { throw test_exception("failure"); };

    stlab::future<int> f = stlab::async(stlab::default_executor, answer);

    BOOST_REQUIRE_EXCEPTION(stlab::get_wait(f), test_exception, ([](const auto& e) {
                                return std::string(e.what()) == std::string("failure");
                            }));
}

BOOST_AUTO_TEST_CASE(future_get_wait_moveonly_value_error_case) {
    BOOST_TEST_MESSAGE("future get_wait with moveonly value");
    auto answer = []() -> stlab::move_only { throw test_exception("failure"); };

    stlab::future<stlab::move_only> f = stlab::async(stlab::default_executor, answer);

    BOOST_REQUIRE_EXCEPTION(stlab::get_wait(std::move(f)), test_exception, ([](const auto& e) {
                                return std::string(e.what()) == std::string("failure");
                            }));
}

BOOST_AUTO_TEST_CASE(future_get_wait_void_error_case) {
    BOOST_TEST_MESSAGE("future get_wait with void error case");
...
BOOST_AUTO_TEST_CASE(future_get_wait_void_error_case_with_timeout) {
    BOOST_TEST_MESSAGE("future get_wait with void error case with timout");
    ...
    BOOST_REQUIRE_EXCEPTION(
        stlab::get_wait_for(f, std::chrono::seconds(60)).get_try(), test_exception,
        ([](const auto& e) { return std::string(e.what()) == std::string("failure"); }));
}

BOOST_AUTO_TEST_CASE(future_get_wait_copyable_value_timeout) {
    BOOST_TEST_MESSAGE("future get_wait with copyable value with timeout");
    auto answer = [] {
    ...
    auto r = stlab::get_wait_for(f, std::chrono::milliseconds(500));
    // timeout should have been reached
    BOOST_REQUIRE(!r.is_ready());
}

BOOST_AUTO_TEST_CASE(future_get_wait_moveonly_value_and_timeout) {
    BOOST_TEST_MESSAGE("future get_wait with moveonly value");
    auto answer = [] {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    ...
    stlab::future<stlab::move_only> f = stlab::async(stlab::default_executor, answer);
    auto r = stlab::get_wait_for(std::move(f), std::chrono::milliseconds(500));
    BOOST_REQUIRE(r.is_ready());
    BOOST_REQUIRE_EQUAL(42, r.get_try()->member());
}

BOOST_AUTO_TEST_CASE(future_get_wait_moveonly_value_error_case_and_timeout) {
    BOOST_TEST_MESSAGE("future get_wait with moveonly value and timeout set");
    auto answer = [] {
    ...
    BOOST_REQUIRE_EXCEPTION(
        stlab::get_wait_for(std::move(f), std::chrono::seconds(500)).get_try(), test_exception,
        ([](const auto& e) { return std::string(e.what()) == std::string("failure"); }));
}
...
BOOST_AUTO_TEST_CASE(future_reduction_with_mutable_task) {
    ...
    BOOST_REQUIRE_EQUAL(2, *stlab::get_wait(result).get_try());
}
...
BOOST_AUTO_TEST_CASE(future_reduction_with_mutable_void_task) {
    ...
    static_cast<void>(stlab::get_wait(result));
...
BOOST_AUTO_TEST_CASE(future_reduction_with_move_only_mutable_task) {
    ...
    BOOST_REQUIRE_EQUAL(2, (*stlab::get_wait(std::move(result)).get_try()).member());
}
...
BOOST_AUTO_TEST_CASE(future_reduction_with_move_only_mutable_void_task) {
    ...
    static_cast<void>(stlab::get_wait(std::move(result)));
