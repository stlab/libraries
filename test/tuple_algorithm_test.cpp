...
BOOST_AUTO_TEST_CASE(one_element_tuple_that_fails)
{
    ...
    BOOST_REQUIRE_EQUAL(std::size_t(1), stlab::tuple_find(t, [](const auto& t) { return !t.empty(); }));
}

BOOST_AUTO_TEST_CASE(one_element_tuple_that_succeeds)
{
    std::tuple<std::vector<int>> t;
    BOOST_REQUIRE_EQUAL(std::size_t(0), stlab::tuple_find(t, [](const auto& t) { return t.empty(); }));
}

BOOST_AUTO_TEST_CASE(two_element_tuple_that_fails)
{
    std::tuple<std::vector<int>, std::list<double>> t;
    BOOST_REQUIRE_EQUAL(std::size_t(2), stlab::tuple_find(t, [](const auto& t) { return !t.empty(); }));
}
